...
.hero-dept { font-size: 0.7rem; font-weight: 600; letter-spacing: 0.2em; text-transform: uppercase; color: var(--gold); margin-bottom: 8px; }
.hero-title { font-family: 'Orbitron', monospace; font-size: 3rem; font-weight: 900; line-height: 1; margin-bottom: 6px; }
.hero-title .t { color: #FFFFFF; }
.hero-title .v { color: var(--gold); }
.hero-tagline { font-size: 1rem; font-weight: 300; color: var(--cyan); font-style: italic; margin-bottom: 18px; }
.hero-desc { font-size: 0.88rem; color: #8A9BBF; line-height: 1.7; max-width: 680px; margin-bottom: 20px; }
.hero-tags { display: flex; flex-wrap: wrap; gap: 8px; }
.hero-tag { font-size: 0.72rem; font-weight: 600; letter-spacing: 0.05em; padding: 5px 12px; border-radius: 20px; border: 1px solid; }
...
.hero-location { position: absolute; top: 24px; right: 28px; font-size: 0.7rem; color: var(--muted); text-align: right; line-height: 1.6; }
...
.stat-label { font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.12em; color: var(--muted); margin-bottom: 8px; }
.stat-value { font-family: 'Orbitron', monospace; font-size: 1.5rem; font-weight: 700; }
.stat-card.gold .stat-value { color: var(--gold); }
.stat-card.cyan .stat-value { color: var(--cyan); }
.stat-card.teal .stat-value { color: var(--teal); }
.stat-card.pink .stat-value { color: var(--pink); }

.stat-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin: 20px 0 24px; }

.badge-row { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 20px; }
.badge { font-size: 0.75rem; font-weight: 600; padding: 5px 14px; border-radius: 20px;
    background: linear-gradient(135deg, rgba(0,201,255,0.15), rgba(0,229,204,0.15));
    border: 1px solid rgba(0,201,255,0.3); color: var(--cyan); }

.sec-label { font-size: 0.68rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.15em;
    color: var(--muted); margin: 20px 0 10px; padding-bottom: 8px; border-bottom: 1px solid var(--border); }

[data-testid="stFileUploader"] { background: var(--card) !important; border: 2px dashed var(--border) !important; border-radius: 14px !important; }
[data-testid="stImage"] img { border-radius: 12px; border: 1px solid var(--border); }

.sidebar-brand { background: linear-gradient(135deg, #0D1B3E, #0A2240); border-bottom: 1px solid var(--border); padding: 24px 20px 20px; margin: -1rem -1rem 20px; }
.sidebar-logo { font-family: 'Orbitron', monospace; font-size: 1.1rem; font-weight: 900; color: #fff; margin-bottom: 4px; }
.sidebar-logo span { color: var(--gold); }
.sidebar-sub { font-size: 0.65rem; color: var(--muted); text-transform: uppercase; letter-spacing: 0.1em; line-height: 1.5; }

.processing-bar { height: 3px; background: linear-gradient(90deg, var(--cyan), var(--teal), var(--gold), var(--pink), var(--cyan));
    background-size: 200% 100%; border-radius: 2px; margin-bottom: 16px; animation: shimmer 1.5s linear infinite; }

.empty-state { text-align: center; padding: 80px 0; animation: pulse 3s ease-in-out infinite; }
.empty-icon { font-size: 3rem; margin-bottom: 14px; }
.empty-text { font-family: 'Orbitron', monospace; font-size: 0.85rem; color: var(--border); letter-spacing: 0.08em; }
...
<div class="hero">
    ...
    <div class="hero-desc">
        ...
        It detects and tracks objects in live video, then explains why the model made each decision,
        how confident it is, and where it is likely to fail.
    </div>
    <div class="hero-tags">
...
if source == "Upload Image":
    ...
    if file:
        ...
        <div class="stat-grid">
            <div class="stat-card pink">
                ...
                <div class="stat-value">{latency:.0f}</div>
            </div>
            <div class="stat-card gold">
                <div class="stat-label">CPU Usage</div>
                <div class="stat-value">{cpu_usage:.0f}%</div>
            </div>
            <div class="stat-card cyan">
                <div class="stat-label">Model</div>
                <div class="stat-value" style="font-size:0.9rem;">{model_choice.replace('.pt','')}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if file:
        ...
        with col2:
            ...
            stat_box = st.empty()
            st.markdown('<div class="sec-label">Classes</div>', unsafe_allow_html=True)
            class_box = st.empty()
        ...
        while cap.isOpened():
            ...
            if frame_count % 10 == 0:
                ...
                avg_fps  = sum(fps_list[-10:]) / len(fps_list[-10:]) if fps_list else 0
                ...
                stat_box.markdown(f"""
                <div class="stat-card gold" style="margin-bottom:10px;">
                    <div class="stat-label">Objects</div>
                    <div class="stat-value">{num_det}</div>
                </div>
                <div class="stat-card cyan" style="margin-bottom:10px;">
                    <div class="stat-label">Avg Conf</div>
                    <div class="stat-value">{avg_c:.0%}</div>
                </div>
                <div class="stat-card teal" style="margin-bottom:10px;">
                    <div class="stat-label">FPS</div>
                    <div class="stat-value">{avg_fps:.1f}</div>
                </div>
                <div class="stat-card pink" style="margin-bottom:10px;">
                    <div class="stat-label">Latency (ms)</div>
                    <div class="stat-value">{latency:.0f}</div>
                </div>
                <div class="stat-card gold">
                    <div class="stat-label">CPU Usage</div>
                    <div class="stat-value">{cpu:.0f}%</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        avg_fps_total = sum(fps_list) / len(fps_list) if fps_list else 0
        cap.release()
        os.unlink(tfile.name)
        st.success(f"Done. Processed {frame_count} frames. Average FPS: {avg_fps_total:.1f}")
